...
function cost = computeRMS(parameters)
    ...
    set_param('springModel/VDS/Kmax', 'constant', num2str(Kmax)); 
    set_param('springModel/VDS/Kmin', 'constant', num2str(Kmin)); 
    set_param('springModel/KL3_damper', 'D', num2str(c)); 
    set_param('springModel/KL3_inerter', 'B', num2str(b));

    % Run the simulation
    simOut = sim('springModel.slx');
...
lower_bounds = [10, 0.1, 1, 0.1];   % [Cmax, Cmin, Spring stiffness, Inerter]
upper_bounds = [100, 10, 5000, 50]; % [Cmax, Cmin, Spring stiffness, Inerter]

% Initial guess for the parameters
initial_guess = [50, 1, 500, 5]; % [Cmax, Cmin, Spring stiffness, Inerter]
...
set_param('NEW_KL3_NEW/KL3_damper', 'D', num2str(optimal_c));
set_param('NEW_KL3_NEW/KL3_interter', 'B', num2str(optimal_b));
