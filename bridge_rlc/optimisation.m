...
function cost = computeRMS(parameters)
    ...
    set_param('rlc_full_two/resistor', 'R', num2str(R_val));
    set_param('rlc_full_two/inductor', 'L', num2str(L_val));
    set_param('rlc_full_two/capacitor', 'C', num2str(C_val));
    set_param('rlc_full_two/diode', 'Ron', num2str(D_Ron));
    set_param('rlc_full_two/diode', 'Vf', num2str(Vf_Ron));

    % Run the simulation
    simOut = sim('rlc_full_two.slx');
    ...
    % Print tracking info
    fprintf('R: %.2f Ω, L: %.4f H, C: %.6f F, D_Ron: %.3f Ω, Vf_Ron: %.2f V, RMS: %.10f\n', ...
        R_val, L_val, C_val, D_Ron, Vf_Ron, cost);
end

% Define parameter bounds
lower_bounds = [1, 1e-1, 1e-2, 0.01, 0.1];   % [R, L, C, D_Ron, Vf_Ron]
upper_bounds = [50, 5, 1, 1, 1];       % [R, L, C, D_Ron, Vf_Ron]

% Initial guess
initial_guess = [8.25, 0.4125, 0.01, 0.983, 0.7];
...
set_param('rlc_full_two/resistor', 'R', num2str(optimal_R));
set_param('rlc_full_two/inductor', 'L', num2str(optimal_L));
set_param('rlc_full_two/capacitor', 'C', num2str(optimal_C));
set_param('rlc_full_two/diode', 'Ron', num2str(optimal_D_Ron));
set_param('rlc_full_two/diode', 'Vf', num2str(optimal_Vf_Ron));
