...
lower_bounds = [1, 1, 1, 1e-4];      % Min values: [R1 (Ω), R2 (Ω), R3 (Ω), D1 Voltage (V), C1 (F)]
upper_bounds = [1000, 1000, 1000, 1e-2]; % Max values: [R1 (Ω), R2 (Ω), R3 (Ω), D1 Voltage (V), C1 (F)]

% Initial guess for parameters
initial_guess = [100, 10, 10, 0.3, 1e-3]; % Example: R1=10Ω, R2=0.001Ω, R3=10Ω, Diode=0.3V, Cap=1μF
...
function cost = computeRMS(parameters)
   ...
   % set_param('VDD_NL0_electrical/R3', 'R', num2str(R3_val));
    ...
    fprintf('R1: %.4f Ω, R2: %.4f Ω, C1: %.6e F, Jr: %.10f\n', ...
        R1_val, R2_val, C1_val, cost);
end
